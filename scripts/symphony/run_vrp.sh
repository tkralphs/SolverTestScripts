...
echo fp_enabled 0 > sym.par

INST=`basename ${NAME%.*}`
NUMROUTES=`echo $INST | cut --delimiter="-" --fields=3 | cut --delimiter="k" --fields=2`

if test $THREADS != 0
then
    $BINNAME -F $NAME -N $NUMROUTES -t $TIMELIMIT -p $THREADS -f sym.par | tee $SOLFILE
else
    $BINNAME -F $NAME.vrp -N $NUMROUTES -t $TIMELIMIT -f sym.par | tee $SOLFILE
fi
